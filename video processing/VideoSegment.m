function VideoSegment (VideoFile)

trafficVid = VideoReader(VideoFile);
%implay('D:\Users\crystal_ship\Downloads\test_avi.avi');

out_vid = '';
for i=1:size(VideoFile,2)-4 
            out_vid(i)=VideoFile(i);
end
out_vid=strcat(out_vid,'_out.avi');


outputVideo = VideoWriter(fullfile(out_vid));
outputVideo.FrameRate = trafficVid.FrameRate;
open(outputVideo);

grid_vid = '';
for i=1:size(VideoFile,2)-4 
            grid_vid(i)=VideoFile(i);
end
grid_vid=strcat(grid_vid,'_grid.avi');

gridVideo = VideoWriter(fullfile(grid_vid));
gridVideo.FrameRate = trafficVid.FrameRate;
open(gridVideo);

cont_vid = '';
for i=1:size(VideoFile,2)-4 
            cont_vid(i)=VideoFile(i);
end
cont_vid=strcat(cont_vid,'_cont.avi');

contourVideo = VideoWriter(fullfile(cont_vid));
contourVideo.FrameRate = trafficVid.FrameRate;
open(contourVideo);


nframes = 10;%trafficVid.NumberOfFrames;
...
%%%%%%%%%%%
   ...
   %     imshow(transparent3) ;
    ...
    writeVideo(gridVideo,grid);
        writeVideo(contourVideo,cont);

     writeVideo(outputVideo,transparent3);
